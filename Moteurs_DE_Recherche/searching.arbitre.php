...
$requete_arbitres = "SELECT DISTINCT a.NOM , a.PRENOM , pr.ID_PERS
    FROM PARTICIPANT p , PERSONNEL pr
    WHERE pr.id_part = p.id_part
    AND pr.TYPE_PERS = 'ARBITRE'
    ";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST["nom_arbitre"])) {
        $requete_arbitres .= " AND ( upper(a.NOM) LIKE upper(:nom_arbitre) OR upper(a.PRENOM) LIKE upper(:nom_arbitre) )";
        $to_bind[] = ":nom_arbitre";
        $the_values[] = '%' . htmlentities(trim($_POST["nom_arbitre"])) . '%';

        setcookie("NOM_ARBITRE_COOKIE", $_POST["nom_arbitre"], strtotime('+1 day'));
    }
    if (isset($_POST["nationalite_arbitre"])) {
        $requete_arbitres .= " AND ( upper(a.NATIONALITE) LIKE upper(:nationalite_arbitre) )";
    ...
    if (isset($_POST["age_min_arbitre"]) && is_numeric($_POST["age_min_arbitre"])) {
        $requete_arbitres .= " AND  ( EXTRACT(YEAR FROM DATE '2024-01-01') - EXTRACT(YEAR FROM a.DATENAISS) ) >= :age_min_arbitre";
    ...
    if (isset($_POST["age_max_arbitre"]) && is_numeric($_POST["age_max_arbitre"])) {
        $requete_arbitres .= " AND ( EXTRACT(YEAR FROM DATE '2024-01-01') - EXTRACT(YEAR FROM a.DATENAISS) ) <= :age_max_arbitre";
...
<body>
    ...
    <form method="POST">
        ...
        <input type="text" id="nationalite" name="nationalite_arbitre" value="<?= $Nationalite_arbitre ?>"><br><br>
        ...
        <select id="competition" name="competition">
            ...
            if ($compet_stid) {
                while ($row = oci_fetch_assoc($compet_stid)) {
                    echo "<option value='{$row["ID_COMPET"]}'> {$row["TYPE_COMPET"]} </option>";
                }
            }
            ?>
        </select><br><br>
